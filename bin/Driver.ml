...
let compile path output_table =
  ...
  let write_ir ir content = if output_table ir then write (ir_path basename ir) content in
  (* lexical analysis *)
...
let command =
  Command.basic
    ...
    ~readme:(fun () -> "")
    ((* define options *)
     let%map_open.Command elaboration =
       flag "-elaboration" no_arg ~doc:"write out elaborated syntax tree"
     and apex = flag "-apex" no_arg ~doc:"write out lifted syntax tree"
