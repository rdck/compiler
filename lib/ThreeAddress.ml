...
let represent_expression = function
  | Lit i -> sprintf "%d" i
  | Bin (op, lhs, rhs) ->
      sprintf "%s %s %s" (represent_binop op) (represent_register lhs) (represent_register rhs)
  | Closure (f, args) ->
      let s = String.concat ~sep:" " (List.map args ~f:represent_register) in
      sprintf "close f%d {%s}" f s
  | Call (f, x) ->
      sprintf "call %s %s" (represent_register f) (represent_register x)
  ...
  | Mat (control, _, environment, cases) ->
      ...
      let environment = String.concat ~sep:" " (List.map environment ~f:represent_register) in
...
let represent_instruction = function
  ...
  | Return r ->
      sprintf "ret %s" (represent_register r)
  | Count (op, r) ->
      sprintf "%s %s" (represent_count_operation op) (represent_register r)
