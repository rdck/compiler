...
let count_term environment arg instructions =
  ...
  let (body, return) = List.split_while instructions ~f:(function
    | Return _ -> false
    | _ -> true
  ) in

  (* save return register for later *)
  let return_register = match List.hd_exn return with
  | Return r -> r
  | _ -> failwith "expected return statement at end of function" in

  (* set up a map of local register types *)
  let term_map =

    (* function environment *)
    let env = List.map environment ~f:(
      fun { name ; value = t } -> (Env name, t)
    ) in

    (* function argument *)
    let env = match arg with
    | None      -> env
    | Some arg  -> (Arg, arg.value) :: env in

    (* local registers *)
    let block = List.filter_map body ~f:(function
      | Store (r, t, _) when is_arrow_type t -> Some (r, t)
      | _ -> None
    ) in

    register_map_of (env @ block) in
  ...
  let body =
    ...
    let stores = List.filter_map body ~f:(function
